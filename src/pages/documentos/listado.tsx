...
import { Table, Tag, Flex, Modal } from "antd";
...
const DocumentosListado = (props: Pick<ControlProps, "filter">) => {
    ...
    const { filter = '' } = props
    const { contextDocumentos: { state, todos, loadFile } } = useData()
    ...
    const { ButtonText } = useComponents()
    const { IconSearchPlus, IconFilePDF } = useIconos()
    ...
    return (
        <>
            <Table
                ...
                <Column render={() => (<IconFilePDF style={{ fontSize: 24, color: Colors.Font.Danger }} />)} />
